...
fn cascade_dead_deps_in_tx(conn: &Connection, now: i64, limit: usize) -> Result<usize> {
    ...
    if remaining > 0 {
        count += upgrade_stale_recoverable_parks(conn, now, remaining)?;
    }
    Ok(count)
}

/// Task #473: a failed dep can be retried to `open` and then cancelled. The
/// dependent was parked with the recoverable `terminal-not-done` reason and
/// `daemon_parked_unsatisfiable=false`; the primary cascade only touches
/// `open`/`rework` rows, so the dependent stays hidden from BLOCKED and
/// durably indistinguishable from a recoverable park even though it is now
/// permanently undispatchable. This second bounded pass re-examines
/// already-parked rows whose `depends_on` currently contains a cancelled dep
/// and upgrades the marker + reason in place — no lifecycle transition, no
/// duplicate owner alert (the original park alert already fired).
fn upgrade_stale_recoverable_parks(conn: &Connection, now: i64, limit: usize) -> Result<usize> {
    ...
    let mut count = 0usize;
    for (task_id, cancelled_dep) in &stale {
        let reason = format!("dependency #{cancelled_dep} is cancelled — unsatisfiable");
        conn.execute(
            "UPDATE tasks
             SET refs = json_set(
                     refs,
...
mod tests {
    ...
    /// Task #473: a failed dep can be retried to `open` and later cancelled.
    /// The dependent was parked with the recoverable `terminal-not-done`
    /// reason and marker=false; without a runtime convergence path it stays
    /// hidden from BLOCKED and durably indistinguishable from a recoverable
    /// park. The sweep must revisit already-failed parked rows and upgrade
    /// them when a dep is now cancelled.
    #[test]
    fn cascade_upgrades_recoverable_park_when_failed_dep_becomes_cancelled() {
        let (_d, mut c) = open_tmp();
        let dep = crate::tasks::create(&mut c, "boss", "dep", None, 0, None, None, None, None, 100)
            .unwrap();
        let child = crate::tasks::create(
        ...
        .unwrap();
        // Original state: dep failed → dependent parked with the recoverable
        // reason and marker=false, exactly as the primary cascade writes it.
        c.execute(
            "UPDATE tasks SET status='failed', updated_at=200 WHERE id=?1",
            params![dep],
        )
        ...
        // Now the dep is retried to open and then cancelled. The dependent
        // remains failed/parked with stale refs; convergence is the sweep's job.
        c.execute(
            "UPDATE tasks SET status='cancelled', updated_at=400 WHERE id=?1",
            params![dep],
        )
        .unwrap();

        // Nothing counted from the primary cascade (dependent is failed, not
        // open/rework), but the second pass upgrades exactly one row.
        assert_eq!(cascade_dead_deps(&c, 500, SWEEP_LIMIT).unwrap(), 1);
        ...
        .unwrap();
        assert_eq!(upgraded["daemon_parked_unsatisfiable"], true);
        assert_eq!(
            upgraded["daemon_parked_reason"],
            serde_json::Value::String(format!("dependency #{dep} is cancelled — unsatisfiable"))
        );
        // Idempotent: second sweep does not re-count / re-emit.
        assert_eq!(cascade_dead_deps(&c, 600, SWEEP_LIMIT).unwrap(), 0);
        // Emits a distinct event kind so operators can find the upgrade in
        // the event stream separately from the original park.
        let evs = crate::events::list(&c, 0, Some(&format!("task#{child}")), 10, 600).unwrap();
        assert!(
