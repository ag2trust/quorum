(() => {
  ...
  globalThis.QuorumWeb = {stripShellWrapper, commandSummary, normalizeEvent};
  ...
  function renderEvent(event) {
    ...
    const entry = normalizeEvent(event), row = document.createElement('div'); row.className = `run-row run-${entry.kind}`;
  ...
  function appendTail(lines, replace) { if (replace) { $('stream').replaceChildren(); renderedChars = 0; rawText = ''; } const text = lines.join('\n') + (lines.length ? '\n' : ''); rawText = (rawText + text).slice(-MAX_RENDERED_TAIL_CHARS); lines.forEach(line => { try { renderEvent(JSON.parse(line)); } catch (_) { renderEvent(null); } }); if (rawMode) renderRaw(); }
  ...
  async function runs(reset = true) { if (document.hidden) return; const suffix = reset || !runsBefore ? '' : '?before=' + encodeURIComponent(runsBefore); const response = await fetch('/api/runs' + suffix).then(x => x.json()); renderRuns(response.runs, reset); runsBefore = response.next_before; $('moreRuns').classList.toggle('hidden', !runsBefore); }
